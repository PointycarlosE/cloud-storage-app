...
2. **Use o Gunicorn** (configuração em `app/gunicorn_config.py`):
...
- [x] Upload múltiplo com drag & drop e painel de progresso em tempo real
- [x] Download individual e em lote (ZIP)
- [x] Exclusão individual sem reload (AJAX)
- [x] Lightbox para imagens, player de áudio e visualizador de PDF
- [x] Pesquisa em tempo real e ordenação de arquivos
...
- [x] Modo Híbrido (Local/Produção) com Gunicorn configurado
